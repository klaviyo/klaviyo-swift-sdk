...
final class GeofenceTests: XCTestCase {
    ...
    func testGeofenceInitializationWithDwell() throws {
        let geofence = try Geofence(
    ...
    func testGeofenceToCLCircularRegionWithDwell() throws {
        let geofence = try Geofence(
            id: "ABC123:8db4effa-44f1-45e6-a88d-8e7d50516a0f:300",
            longitude: -122.03026995144546,
            latitude: 37.33204742438631,
    ...
    func testGeofenceServiceAppendsCompanyIdToUUID() async throws {
        ...
        XCTAssertNil(firstGeofence.dwell)
        ...
        XCTAssertNil(secondGeofence.dwell)
    }

    func testGeofenceServiceWithDwell() async throws {
        // Given
        KlaviyoLocationTestUtils.setupTestEnvironment(apiKey: "ABC123")
        let testData = KlaviyoLocationTestUtils.createTestGeofenceDataWithDwell()
        let geofenceService = GeofenceService()
        ...
        XCTAssertEqual(firstGeofence.companyId, "ABC123")
        XCTAssertEqual(firstGeofence.dwell, 300)

        let secondGeofence = geofences.first { $0.locationId == "a84011cf-93ef-4e78-b047-c0ce4ea258e4" }!
        XCTAssertEqual(secondGeofence.id, "ABC123:a84011cf-93ef-4e78-b047-c0ce4ea258e4:600")
        XCTAssertEqual(secondGeofence.companyId, "ABC123")
        XCTAssertEqual(secondGeofence.dwell, 600)
    }

    func testGeofenceServiceWithMixedDwell() async throws {
        // Given
        KlaviyoLocationTestUtils.setupTestEnvironment(apiKey: "ABC123")
        let testData = KlaviyoLocationTestUtils.createTestGeofenceDataWithMixedDwell()
        let geofenceService = GeofenceService()
    ...
    // MARK: - Dwell Parsing Tests

    func testDwellParsing() throws {
        let geofenceWithDwell = try Geofence(
            id: "ABC123:8db4effa-44f1-45e6-a88d-8e7d50516a0f:300",
            longitude: -74.006,
            latitude: 40.7128,
            radius: 100.0
        )
        XCTAssertEqual(geofenceWithDwell.dwell, 300)

        let geofenceWithoutDwell = try Geofence(
            id: "ABC123:8db4effa-44f1-45e6-a88d-8e7d50516a0f:",
            longitude: -74.006,
            latitude: 40.7128,
            radius: 100.0
        )
        XCTAssertNil(geofenceWithoutDwell.dwell)
    }

    func testDwellParsingWithLargeValue() throws {
        let geofence = try Geofence(
        ...
        XCTAssertEqual(geofence.dwell, 999_999)
    }

    func testDwellParsingWithZero() throws {
        let geofence = try Geofence(
        ...
        XCTAssertEqual(geofence.dwell, 0)
    }

    // MARK: - CLCircularRegion Extension Tests

    func testCLCircularRegionToKlaviyoGeofenceWithDwell() throws {
    ...
    func testCLCircularRegionToKlaviyoGeofenceWithoutDwell() throws {
        let region = CLCircularRegion(
            center: CLLocationCoordinate2D(latitude: 40.7128, longitude: -74.006),
            radius: 100.0,
    ...
    func testKlaviyoDwellExtension() throws {
        let region = CLCircularRegion(
            center: CLLocationCoordinate2D(latitude: 40.7128, longitude: -74.006),
            radius: 100.0,
            identifier: "ABC123:8db4effa-44f1-45e6-a88d-8e7d50516a0f:300"
        ...
        XCTAssertEqual(region.klaviyoDwell, 300)
    }

    func testKlaviyoDwellExtensionWithoutDwell() throws {
        let region = CLCircularRegion(
            center: CLLocationCoordinate2D(latitude: 40.7128, longitude: -74.006),
            radius: 100.0,
            identifier: "ABC123:8db4effa-44f1-45e6-a88d-8e7d50516a0f:"
        ...
        XCTAssertNil(region.klaviyoDwell)
    }

    func testKlaviyoDwellExtensionWithInvalidIdentifier() {
        let region = CLCircularRegion(
            center: CLLocationCoordinate2D(latitude: 40.7128, longitude: -74.006),
            radius: 100.0,
