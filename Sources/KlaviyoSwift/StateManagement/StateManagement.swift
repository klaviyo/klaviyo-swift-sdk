        ...
        Reduce { state, action in
            if action.requiresInitialization,
               case .uninitialized = state.initalizationState {
                ...
                return Effect<KlaviyoAction>.none
            }
            ...
            case var .completeInitialization(initialState):
                ...
                .merge(with: .run { send in
                    for await value in environment.appLifeCycle.lifeCycleEvents().map(\.transformToKlaviyoAction).values {
                        await send(value)
                    }
                })
                .merge(with: .run { send in
                    for await value in klaviyoSwiftEnvironment.stateChangePublisher().values {
                        await send(value)
                    }
                })
            ...
            case .stop:
                ...
                return Effect.cancel(id: CancelId.request)
            ...
            case .start:
                ...
                return .merge([
                    ...
                    .run { [state] send in
                        for await flush in environment.timer(state.flushInterval)
                            .map({ _ in
            ...
            case let .deQueueCompletedResults(completedRequest):
                ...
                return .run { send in await send(.sendRequest) }.cancellable(id: CancelId.request)
            ...
            case let .networkConnectivityChanged(networkStatus):
                ...
                case .notReachable:
                    ...
                    return Effect.cancel(id: CancelId.request).concatenate(with: .cancel(id: CancelId.timer))
            ...
            case var .enqueueEvent(event):
                 ...
                 */
                return event.metric.name == ._openedPush ? .run { send in await send(.flushQueue) } : .none
            ...
            case let .resetStateAndDequeue(request, invalidFields):
                ...
                return .run { send in await send(.deQueueCompletedResults(request)) }
            }
        }
    }
}
