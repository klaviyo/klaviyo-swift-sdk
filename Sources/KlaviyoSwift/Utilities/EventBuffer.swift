...
fileprivate extension Logger {
    static let eventBuffer = Logger(subsystem: "com.klaviyo.klaviyo-swift-sdk.klaviyoSwift", category: "Event Buffering")
}
        ...
        queue.sync {
            ...
            if #available(iOS 14.0, *) {
                if recentEvents.isEmpty {
                    ...
                    Logger.eventBuffer.info("📬 Replaying \(recentEvents.count) buffered event(s): \(recentEvents.map { $0.metric.name.value }.joined(separator: ", "), privacy: .public)")
                }
            }
