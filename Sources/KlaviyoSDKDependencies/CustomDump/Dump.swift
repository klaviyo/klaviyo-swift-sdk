...
func _customDump<T, TargetStream>(
  ...
  func customDumpHelp<InnerT, InnerTargetStream>(
    ...
    case let (value as Any.Type, _):
      out.write("\(customDumptypeName(value)).self")
    ...
    case let (value as _CustomDiffObject, _):
      ...
      let subjectType = customDumptypeName(type(of: value))
    ...
    case let (value as AnyObject, .class?):
      ...
      var occurrence = tracker.occurrencePerType[customDumptypeName(mirror.subjectType), default: 0] {
        didSet { tracker.occurrencePerType[customDumptypeName(mirror.subjectType)] = occurrence }
      }
      ...
      if tracker.visitedItems.contains(item) {
        out.write("\(customDumptypeName(mirror.subjectType))(↩︎)")
      } else {
        tracker.visitedItems.insert(item)
        occurrence += 1
        var children = Array(mirror.children)
        ...
        dumpChildren(
          ...
          prefix: "\(customDumptypeName(mirror.subjectType))(",
          suffix: ")",
          shouldSort: false,
    ...
    case (_, .enum?):
      out.write(isRoot ? "\(customDumptypeName(mirror.subjectType))." : ".")
