  ...
  func diffHelp(
    ...
    func diffEverything() {
      ...
      if lhs == rhs {
        if lhsMirror.subjectType != rhsMirror.subjectType {
          lhs.append(" as \(customDumptypeName(lhsMirror.subjectType))")
          rhs.append(" as \(customDumptypeName(rhsMirror.subjectType))")
        }
      }
    ...
    case let (lhs as _CustomDiffObject, _, rhs as _CustomDiffObject, _):
      ...
      if lhsItem == rhsItem {
        ...
        let subjectType = customDumptypeName(type(of: lhs))
    ...
    case let (lhs as AnyObject, .class?, rhs as AnyObject, .class?):
      ...
      let subjectType = customDumptypeName(lhsMirror.subjectType)
    ...
    case (_, .enum?, _, .enum?):
      ...
      let subjectType = isRoot ? customDumptypeName(lhsMirror.subjectType) : ""
      diffChildren(
    ...
    case (_, .struct?, _, .struct?):
      diffChildren(
        lhsMirror,
        rhsMirror,
        prefix: "\(customDumptypeName(lhsMirror.subjectType))(",
        suffix: ")",
        elementIndent: 2,
        elementSeparator: ",",
        collapseUnchanged: false,
