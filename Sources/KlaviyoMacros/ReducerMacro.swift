...
extension ReducerMacro: ExtensionMacro {
    public static func expansion<D: DeclGroupSyntax, T: TypeSyntaxProtocol, C: MacroExpansionContext>(
        ...
        in context: C) throws -> [ExtensionDeclSyntax] {
...
extension ReducerMacro: MemberAttributeMacro {
    public static func expansion<D: DeclGroupSyntax, M: DeclSyntaxProtocol, C: MacroExpansionContext>(
        ...
        in context: C) throws -> [AttributeSyntax] {
            ...
            return [
                AttributeSyntax(
                    attributeName: IdentifierTypeSyntax(
                        name: .identifier("ReducerBuilder<\(genericArguments)>")
                    )
                )
            ]
...
extension IdentifierTypeSyntax {
    fileprivate var isEphemeral: Bool {
        name.text == "AlertState" || name.text == "ConfirmationDialogState"
    }
}
...
extension ReducerMacro: MemberMacro {
    public static func expansion<D: DeclGroupSyntax, C: MacroExpansionContext>(
        ...
        in context: C) throws -> [DeclSyntax] {
        ...
        if let enumDecl = declaration.as(EnumDeclSyntax.self) {
            ...
            if !hasState {
                ...
                decls.append(
                    ...
                    @dynamicMemberLookup
                    @ObservableState
                    \(access)enum State: CaseReducerState\
            ...
            if !hasBody {
                ...
                if reducerScopes.isEmpty {
                    body.append(
                        """
                        EmptyReducer<Self.State, Self.Action>()
                        """
                    )
                } else {
                    body.append(
                ...
                decls.append(
                    """
                    @ReducerBuilder<Self.State, Self.Action>
        ...
        case let .element(element, attribute):
            if attribute == nil,
               ...
               parameter.type.is(IdentifierTypeSyntax.self) || parameter.type.is(MemberTypeSyntax.self) {
                ...
                return .scoped(["Scope<Self.State, Self.Action, \(type.trimmed)>"])
            } else {
                return .scoped([])
            }
        case .ifConfig:
        ...
        case let .element(element, attribute):
            if attribute == nil,
               ...
               parameter.type.is(IdentifierTypeSyntax.self) || parameter.type.is(MemberTypeSyntax.self) {
                let name = element.name.text
                let type = parameter.type
                return "case \(name)(StoreOf<\(type.trimmed)>)"
            } else {
                return "case \(element.trimmedDescription)"
            }
        case let .ifConfig(configs):
...
extension EnumCaseDeclSyntax {
    ...
    private var isIgnored: Bool {
        attributes.contains("ReducerCaseIgnored")
            || elements.contains { $0.parameterClause?.parameters.count != 1 }
    }

    fileprivate var isEphemeral: Bool {
        attributes.contains("ReducerCaseEphemeral")
            || elements.contains {
                guard
