...
public class Klaviyo : NSObject {
    ...
    */
    private override init() {
        ...
        // Create the queue
        serialQueue = DispatchQueue(label: "com.klaviyo.serialqueue") //dispatch_queue_create("com.klaviyo.serialQueue", DISPATCH_QUEUE_SERIAL)
    ...
    private func assertPropertyTypes(properties: NSDictionary?) {
        ...
        for (k, _) in properties! {
            assert(properties![k as! NSCopying]! is NSString || properties![k as! NSCopying]! is NSNumber || properties![k as! NSCopying]! is NSNull || properties![k as! NSCopying]! is NSArray || properties![k as! NSCopying]! is NSDictionary
                || properties![k as! NSCopying]! is NSDate || properties![k as! NSCopying]! is NSURL
    ...
    func addNotificationObserver() {
        ...
        notificationCenter.addObserver(self, selector: #selector(Klaviyo.applicationDidBecomeActiveNotification(notification:)), name: NSNotification.Name.UIApplicationDidBecomeActive, object: nil)
        notificationCenter.addObserver(self, selector: #selector(Klaviyo.applicationDidEnterBackgroundNotification(notification:)), name: NSNotification.Name.UIApplicationDidEnterBackground, object: nil)
        notificationCenter.addObserver(self, selector: #selector(Klaviyo.hostReachabilityChanged(note:)), name: ReachabilityChangedNotification, object: nil)
    ...
    */
    private func encodeAPIParamData(dict: NSDictionary)->String {
        var b64String = ""
        let data : Data? = JSONSerializeObject(obj: dict)
     ...
     */
    private func JSONSerializeObject(obj : AnyObject)-> Data? {
    ...
    private func JSONSerializableObjectForObject(obj: AnyObject)-> AnyObject {
        ...
        if obj is NSArray {
            ...
            for item in objects {
                a.add(JSONSerializableObjectForObject(obj: item as AnyObject))
            }
            return a as NSArray
        }
        ...
        let s: NSString = obj.description as NSString
        return s
    }
